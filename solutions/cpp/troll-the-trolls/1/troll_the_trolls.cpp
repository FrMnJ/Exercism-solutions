    ...
    bool valid_player_combination(AccountStatus user1, AccountStatus user2){
        switch(user1){
            ...
            case AccountStatus::guest:
                return false;
            default:
                return user2!=AccountStatus::troll and user2!=AccountStatus::guest;
        }
    }
...
// account has a strictly higher priority than the second.
    bool has_priority(AccountStatus user, AccountStatus other_user){
        return static_cast<int>(user) > static_cast<int>(other_user);
    }
