...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const montlyRate = dailyRate * 22;
  const discountedMonthRate = montlyRate * (1 - discount);
  const costForMonths = fullMonths * discountedMonthRate;
  const costForRemainingDays = remainingDays * dailyRate;
  return Math.ceil(costForMonths + costForRemainingDays);
}
